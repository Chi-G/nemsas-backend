...
class CRUDMonitoring:
    ...
    async def get_monthly_aggregates(self, db: AsyncSession, year: Optional[int] = None, state_id: Optional[int] = None):
        base_stmt = select(
            ...
            func.sum(Monitoring.als).label("als"),
            func.sum(Monitoring.helicopters).label("helicopters"),
            func.sum(Monitoring.community_volunteers).label("communityVolunteers"),
            ...
            func.sum(Monitoring.paediatric_over_5).label("paediatricOver5"),
            func.sum(Monitoring.drowning).label("drowning"),
            func.sum(Monitoring.snake_bite).label("snakeBite"),
            func.sum(Monitoring.other_weapons).label("otherWeapons"),
            func.sum(Monitoring.gunshot).label("gunshot"),
            func.sum(Monitoring.others).label("others"),
            func.sum(Monitoring.paediatric_under_5).label("paediatricUnder5"),
            func.sum(Monitoring.neonatal_under_5).label("neonatalUnder5"),
            func.sum(Monitoring.obstetric_accident).label("obstetricAccident"),
            func.sum(Monitoring.rta).label("rta"),
